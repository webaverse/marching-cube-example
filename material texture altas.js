...
const grassTexture = textureLoader.load(`${import.meta.url.replace(/(\/)[^\/]*$/, '$1')}/textures/test.png`)
...
#include <skinning_pars_vertex>
    ...
    vec2 textureAtlas2dUV(in vec2 uv,in float width,in float id) {
        float resolution = 1.0 / width;
        vec2 suv =  fract(uv) * resolution;
        float sid = round(id);
        float uvx = floor(mod(sid, width)) * resolution;;
        float uvy = floor(sid / width) * resolution;;
        suv.x += uvx;
        suv.y += uvy; 
        suv.y = 1.0-suv.y;
        return suv;
...
void main() {
    ...
    #include <worldpos_vertex>
        ...
        xaxis = texture2D(grassTexture, textureAtlas2dUV(vtriCoord.xz*0.01, 16.0, vbiome));
        yaxis = texture2D(grassTexture, textureAtlas2dUV(vtriCoord.xz*0.01, 16.0, vbiome));
        zaxis = texture2D(grassTexture, textureAtlas2dUV(vtriCoord.xy*0.01, 16.0, vbiome));
        terrainColor= xaxis * blending.x + yaxis * blending.y + zaxis * blending.z; 
    #endif
...
void main() {
    ...
    #include <map_fragment>
        ...
        // yaxis = texture(terrainArrayTexture, vec3( vtriCoord.xz*0.1, round(vbiome)));

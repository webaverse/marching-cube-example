            ...
            for (let j = -2; j <= 1; j++) {
                ...
                if (enableChunks[key]) {
                    ...
                    if (levelCache[key]) {
                        ...
                        chunk = new Chunk(new Vector3(px, 0, pz), chunkSize, level, ix, iz, this.geometryUtils);
                        levelChunk[chunk.key] = chunk;
                        this.add(chunk)
                    }
                }
    ...
    buildLevelChunk(level, cx = 0, cz = 0) {
        ...
        for (let i = -2; i <= 1; i++) {
            for (let j = -2; j <= 1; j++) {
                ...
                chunk = new Chunk(new THREE.Vector3(x, 0, z), chunkSize, level, ix, iz, this.geometryUtils);

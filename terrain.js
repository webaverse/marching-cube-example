...
export default class Terrain extends THREE.Object3D {
    constructor(physics, geometryUtils) {
        ...
        for (let i = -2; i <= 1; i++) {
            for (let j = -2; j <= 1; j++) {
                const chunck = new Chunck(new THREE.Vector3(i * this.chunkSize, 0, j * this.chunkSize), this.chunkSize, this.geometryUtils);
                this.add(chunck);
    ...
    buildLevelBoundChunk(level = 2) {
        ...
        for (let i = -2; i < 2; i++) {
            const chunck1 = new Chunck(new THREE.Vector3(size, 0, size * i), size, this.geometryUtils);
            this.add(chunck1);
            this.levelChunk[level].push(chunck1);
            const chunck2 = new Chunck(new THREE.Vector3(-lsize, 0, size * i), size, this.geometryUtils);
            this.add(chunck2);
            this.levelChunk[level].push(chunck2);
        }
        for (let i = -1; i < 1; i++) {
            const chunck1 = new Chunck(new THREE.Vector3(size * i, 0, size), size, this.geometryUtils);
            this.add(chunck1);
            this.levelChunk[level].push(chunck1);
            const chunck2 = new Chunck(new THREE.Vector3(size * i, 0, -lsize), size, this.geometryUtils);
            this.add(chunck2);
            this.levelChunk[level].push(chunck2);
        }
    }
